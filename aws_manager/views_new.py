...
def index(request):
    ...
    for resource in RESOURCE_FILES.keys():
        resources[resource] = check_resource_exists(resource)
...
def create_resource(request):
    ...
    # Create temp directory based on resource type
    temp_dir = os.path.join(TERRAFORM_DIR, 'temp_' + resource)
    os.makedirs(temp_dir, exist_ok=True)
    
    try:
        # Read the template Terraform file content
        with open(os.path.join(TERRAFORM_DIR, tf_file), 'r') as src:
            tf_content = src.read()
        ...
        latest_terminal_output = combined_output
...
def get_terminal_output(request):
    """
    Endpoint to get the latest terminal output.
    This is a simple endpoint that returns a simulated output for polling.
    """
    # Simulate terminal output - in a real implementation, 
    # you would have some way to get the actual current output
    return JsonResponse({
        'success': True,
        'output': "Real-time output is not available. The full result will be displayed when the operation completes."
    })
